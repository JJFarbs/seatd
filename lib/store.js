...
export const S = {
  ...
  groups: [], unread: {}, activeGroup: null, activeJoinPass: null,
  notifs: [
...
export async function ensureProfiles(ids) {
  ...
  const { data } = await sb.from('profiles').select('id,name,handle,bio,g,created_at,avatar_url').in('id', missing);
  (data || []).forEach((p) => {
    people[p.id] = { id: p.id, name: p.name || 'Member', handle: p.handle || '@member', bio: p.bio || '', g: Array.isArray(p.g) && p.g.length === 2 ? p.g : ['#2BD4FF', '#7A4DFF'], online: false, since: p.created_at, avatar: p.avatar_url || null };
  });
}
...
async function loadGroups() {
  if (!S.session) { S.groups = []; return; }
  const uid = S.session.user.id;
  try {
    const gm = await sb.from('chat_group_members').select('group_id');
    const gids = [...new Set((gm.data || []).map((x) => x.group_id))];
    if (!gids.length) { S.groups = []; return; }
    const [gs, msgs] = await Promise.all([
      sb.from('chat_groups').select('*').in('id', gids).order('created_at', { ascending: false }),
      sb.from('group_messages').select('*').in('group_id', gids).order('created_at'),
    ]);
    S.groups = (gs.data || []).map((g) => ({
      id: g.id, name: g.name, ownerId: g.owner_id,
      msgs: (msgs.data || []).filter((m) => m.group_id === g.id).map((m) => ({ f: m.from_id === uid ? 'me' : 'them', name: m.name, t: m.text })),
    }));
  } catch (_e) {}
}
export async function createTableGroup(req) {
  if (!S.dbReady || !S.session) return;
  const invited = (req.invited || []).filter((id) => people[id]);
  if (!invited.length) return;
  const gid = 'g-' + req.id;
  const gname = `${people.me.name || 'My'}'s table`;
  const firstMsg = `Group created for ${req.venue} — table ${req.table} 🎉`;
  try {
    const g = await sb.from('chat_groups').insert({ id: gid, name: gname, booking_id: req.id, owner_id: S.session.user.id });
    if (g.error) throw g.error;
    const m = await sb.from('chat_group_members').insert([
      { group_id: gid, user_id: S.session.user.id },
      ...invited.map((id) => ({ group_id: gid, user_id: id })),
    ]);
    if (m.error) throw m.error;
    await sb.from('group_messages').insert({ group_id: gid, from_id: S.session.user.id, name: people.me.name, text: firstMsg });
    mutate((s) => { s.groups.unshift({ id: gid, name: gname, ownerId: S.session.user.id, msgs: [{ f: 'me', name: people.me.name, t: firstMsg }] }); });
  } catch (e) {
    toast('Group chat could not be created: ' + (e?.message || 'unknown error'));
  }
}
export function sendGroupChat(gid, text) {
  const g = S.groups.find((x) => x.id === gid);
  if (!g || !text.trim() || !S.session) return;
  mutate(() => { g.msgs.push({ f: 'me', name: people.me.name, t: text.trim() }); });
  sb.from('group_messages').insert({ group_id: gid, from_id: S.session.user.id, name: people.me.name, text: text.trim() }).then(() => {}, () => {});
}
export async function refreshGroup(gid) {
  if (!S.session) return;
  const uid = S.session.user.id;
  const g = S.groups.find((x) => x.id === gid);
  if (!g) return;
  const { data } = await sb.from('group_messages').select('*').eq('group_id', gid).order('created_at');
  if (data && data.length !== g.msgs.length) {
    mutate(() => { g.msgs = data.map((m) => ({ f: m.from_id === uid ? 'me' : 'them', name: m.name, t: m.text })); });
  }
}
// unread tracking + in-app popup
function bumpUnread(key, viewing, label, text) {
  if (viewing) return;
  mutate((s) => { s.unread[key] = (s.unread[key] || 0) + 1; });
  toast(`💬 ${label}: ${text.length > 48 ? text.slice(0, 48) + '…' : text}`);
}
export function clearUnread(key) {
  if (S.unread[key]) mutate((s) => { s.unread[key] = 0; });
}
...
export async function searchProfiles(q) {
  ...
  if (!term) return [];
  const { data } = await sb.from('profiles').select('id,name,handle,bio,g,avatar_url')
    .or(`name.ilike.%${term}%,handle.ilike.%${term}%`)
    .neq('id', S.session.user.id).limit(5);
  (data || []).forEach((p) => {
    if (!people[p.id]) people[p.id] = { id: p.id, name: p.name, handle: p.handle, bio: p.bio || '', g: Array.isArray(p.g) && p.g.length === 2 ? p.g : ['#2BD4FF', '#7A4DFF'], online: false, avatar: p.avatar_url || null };
  });
  return data || [];
}
    ...
    if (S.session) {
      ...
      if (m.data) {
        ...
        if (m.data.length !== total) {
          ...
          // pop up for new incoming messages
          Object.keys(convos).forEach((o) => {
            const grew = convos[o].length - (S.convos[o] || []).length;
            const last = convos[o][convos[o].length - 1];
            if (grew > 0 && last && last.f === 'them') {
              bumpUnread('c:' + o, S.screen === 'chat' && S.activeChat === o, people[o]?.name || 'New message', last.t);
            }
          });
          mutate((s) => { s.convos = convos; });
        }
      }
      // group chats: new messages + being added to a group
      const prevGroups = S.groups;
      await loadGroups();
      S.groups.forEach((g) => {
        const old = prevGroups.find((x) => x.id === g.id);
        const last = g.msgs[g.msgs.length - 1];
        if (!old && g.ownerId !== uid) {
          bumpUnread('g:' + g.id, false, g.name, "you've been added to the table group");
        } else if (old && g.msgs.length > old.msgs.length && last && last.f === 'them') {
          bumpUnread('g:' + g.id, S.screen === 'groupchat' && S.activeGroup === g.id, `${last.name} · ${g.name}`, last.t);
        }
      });
      mutate(() => {});
    }
...
function applyProfile(profile) {
  ...
  people.me.avatar = profile.avatar_url || null;
...
export async function uploadAvatar(file) {
  if (!S.session || !file) { toast('Log in to set a photo'); return; }
  try {
    ...
    const path = S.session.user.id + '.jpg';
    ...
    await sb.from('profiles').update({ avatar_url: url }).eq('id', S.session.user.id);
    mutate((s) => { people.me.avatar = url; if (s.profile) s.profile.avatar_url = url; });
...
export const tablesFor = (id) => {
  if (!S.tables[id]) {
    const v = venue(id);
    const base = v && Array.isArray(v.layout) && v.layout.length ? v.layout.map((t) => ({ ...t })) : makeTables();
    if (S.dbReady) base.forEach((t) => { t.booked = false; }); // demo pre-bookings don't apply to real venues
    S.tables[id] = base;
  }
  const ts = S.tables[id];
  if (S.dbReady) {
    // "booked" is derived from real confirmed bookings, never stored
    ts.forEach((t) => {
      t.booked = S.requests.some((r) => r.vid === id && r.table === t.id && (r.status === 'confirmed' || r.status === 'checkedin'));
    });
  }
  return ts;
};
...
export function saveVenueLocation(v, lat, lng) {
  mutate(() => { v.lat = lat; v.lng = lng; });
  if (S.dbReady) api('/api/venues', { method: 'PATCH', body: JSON.stringify({ id: v.id, lat, lng }) });
  toast('Location saved — your pin is on the map');
}
export function saveVenueLayout(v, tables) {
  const normalized = tables.slice(0, FLOOR_SLOTS.length).map((t, i) => ({ ...t, ...FLOOR_SLOTS[i], booked: false }));
  mutate((s) => {
...
export const appBase = () => (typeof window !== 'undefined' ? window.location.origin : 'https://www.seatedbookings.com');
export const bookingLink = (r) => appBase() + '/?invite=' + encodeURIComponent(r.code || r.id);
export const profileLink = (p) => appBase() + '/?add=' + encodeURIComponent((p.handle || '').replace(/^@/, ''));

// ---------- deep links (?invite=REF / ?add=handle) ----------
export function captureDeepLink() {
  if (typeof window === 'undefined') return;
  const q = new URLSearchParams(window.location.search);
  const invite = q.get('invite'), add = q.get('add');
  if (!invite && !add) return;
  S.deepLink = { invite, add };
...
export async function applyDeepLink() {
  ...
  if (!d) return;
  if (d.invite) {
    const r = S.requests.find((x) => (x.code || '').toUpperCase() === d.invite.toUpperCase());
    if (!r) { if (S.dbReady) { S.deepLink = null; toast('That invite link points to a booking that no longer exists'); } return; }
    if (S.session) {
      S.deepLink = null;
      mutate((s) => { if (!s.role) s.role = 'user'; s.tab = 'bookings'; s.screen = 'home'; });
      toast(`Invite: ${r.who}'s table at ${r.venue} — ${r.table}`);
    } else {
      toast(`You're invited to ${r.who}'s table at ${r.venue} — log in to join`);
    }
  } else if (d.add && S.session) {
    const term = d.add.replace(/^@/, '');
    const res = await searchProfiles(term);
    const p = (res || []).find((x) => (x.handle || '').toLowerCase().replace('@', '') === term.toLowerCase()) || (res || [])[0];
    if (p) {
      S.deepLink = null;
      mutate((s) => { if (!s.role) s.role = 'user'; s.viewFriend = p.id; s.tab = 'messages'; s.screen = 'friendprofile'; });
    }
  }
}
...
export function decideJoin(ptid, jrid, ok) {
  ...
  if (S.dbReady && S.session) {
    ...
    // approved joiners land in the table's group chat
    if (ok && pt.bookingId) {
      const gid = 'g-' + pt.bookingId;
      const gname = `${people.me.name || 'My'}'s table`;
      sb.from('chat_groups').upsert({ id: gid, name: gname, booking_id: pt.bookingId, owner_id: S.session.user.id }, { onConflict: 'id', ignoreDuplicates: true })
        .then(() => sb.from('chat_group_members').upsert([
          { group_id: gid, user_id: S.session.user.id },
          { group_id: gid, user_id: j.userId },
        ], { onConflict: 'group_id,user_id', ignoreDuplicates: true }))
        .then(() => sb.from('group_messages').insert({ group_id: gid, from_id: S.session.user.id, name: people.me.name, text: `${j.name} joined the table 🙌` }))
        .then(() => loadGroups().then(() => mutate(() => {})))
        .catch(() => {});
    }
  }
}
export function submitBooking() {
  ...
  createTableGroup(req); // everyone on the table lands in a group chat
}
...
// Validate a scanned QR payload and check the guest in if it's legit.
// Returns { ok, msg, name?, table?, party?, ref, already? } for the staff screen.
export function processScan(text) {
  let payload = null;
  try { payload = JSON.parse(text); } catch (_e) {}
  const ref = String(payload?.ref || text || '').trim();
  if (!ref) return { ok: false, msg: 'Unreadable code', ref: '—' };
  const v = clubVenue();
  // joined-table passes (JN-XXXXXX)
  if (ref.toUpperCase().startsWith('JN-')) {
    const suffix = ref.slice(3).replace(/[^a-z0-9]/gi, '').toLowerCase();
    for (const pt of S.publicTables) {
      const j = (pt.joinRequests || []).find((x) => String(x.id).replace(/[^a-z0-9]/gi, '').slice(-6).toLowerCase() === suffix);
      if (j) {
        if (pt.vid !== v.id) return { ok: false, msg: `Pass is for ${pt.venue}, not this venue`, ref };
        if (j.status === 'approved') return { ok: true, msg: `Joining ${pt.owner}'s table`, name: j.name, table: pt.table, ref };
        return { ok: false, msg: `Join request is ${j.status}`, name: j.name, ref };
      }
    }
    return { ok: false, msg: 'Pass not found', ref };
  }
  // booking passes
  const r = S.requests.find((x) => (x.code || '').toUpperCase() === ref.toUpperCase());
  if (!r) return { ok: false, msg: 'Booking not found', ref };
  if (r.vid !== v.id) return { ok: false, msg: `Pass is for ${r.venue}, not this venue`, ref };
  if (r.status === 'checkedin') return { ok: false, already: true, msg: 'Already checked in', name: r.who, table: r.table, ref };
  if (r.status === 'pending') return { ok: false, msg: 'Payments/approval not complete', name: r.who, table: r.table, ref };
  if (r.status !== 'confirmed') return { ok: false, msg: `Booking is ${r.status}`, ref };
  mutate(() => { r.status = 'checkedin'; notify('check', `${r.who} checked in — table ${r.table}.`); });
  syncBooking(r, { status: 'checkedin' });
  if (r.userId) notifyUser(r.userId, 'check', `Welcome to ${r.venue} — you're checked in. Table ${r.table} is yours 🎉`);
  return { ok: true, msg: 'Send to table', name: r.who, table: r.table, party: (r.men || 0) + (r.women || 0), ref };
}
